...
async def test_openai_embedding_provider_generate_embeddings(mock_config):
    ...
    with (
        ...
        patch('core.providers.openai_embedding.AsyncQdrantClient') as mock_qdrant_client
    ):
        ...
        with patch('core.providers.openai_embedding.SentenceSplitter') as mock_splitter_class:
...
async def test_huggingface_embedding_provider_init(mock_config):
    ...
    with patch('core.providers.huggingface_embedding.SentenceTransformer') as mock_sentence_transformer_class:
        mock_sentence_transformer_instance = Mock()
        mock_sentence_transformer_class.return_value = mock_sentence_transformer_instance
        ...
        mock_sentence_transformer_class.assert_called_once_with(mock_config.embedding_model)
        assert provider.config == mock_config