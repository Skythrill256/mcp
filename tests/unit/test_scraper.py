...
async def test_web_scraper_scrape_without_crawler():
    ...
    with pytest.raises(ScrapingError, match="Use async context manager to initialize the scraper"):
...
def test_web_scraper_is_internal_link(mock_config):
    ...
    # Test external links
    assert not scraper._is_internal_link("https://example.com", "https://other.com/page")
    
    # Test edge cases
    assert scraper._is_internal_link("https://example.com", "")  # Empty URL treated as internal