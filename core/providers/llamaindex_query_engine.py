...
class LlamaIndexQueryEngine(QueryEngineProvider):
    ...
    def _setup_llamaindex(self):
        ...
        if self.config.embedding_provider.lower() == "openai":
            ...
            token = self.config.huggingface_api_key or os.environ.get(
                "HUGGING_FACE_HUB_TOKEN"
            )
            embed_model = HuggingFaceEmbedding(
                model_name=self.config.embedding_model, token=token
            )
            # Set the embedding dimensions if specified
            if (
                hasattr(embed_model, "embedding_dimension")
                and self.config.embedding_dimensions
            ):
    ...
    async def initialize(self):
        ...
        try:
            ...
            retriever = VectorIndexRetriever(
                index=self.index, similarity_top_k=10, vector_store_query_mode="default"
            )
    ...
    async def query(
        ...
        try:
            ...
            response = await self.query_engine.aquery(question)  # type: ignore[attr-defined]
            ...
            if include_sources and hasattr(response, "source_nodes"):
                for node in response.source_nodes:
                    if isinstance(node, NodeWithScore):
                        ...
                        cast_list = result["sources"]
                        if isinstance(cast_list, list):
                            cast_list.append(source_info)

                # Apply reranking if enabled and available
                if use_reranking and self.reranker and result["sources"]:
                    try:
                        logger.info(
                            f"Applying Cohere reranking to {len(result['sources'])} sources"
                        )
                        # Rerank the sources
                        reranked_sources = self.reranker.rerank_documents(
                            query=question, documents=result["sources"], top_n=top_k
                        )

                        # Update sources with reranked results
                        reranked_source_list: List[Dict[str, Any]] = []
                        ...
                        logger.warning(
                            f"Reranking failed, using original sources: {str(e)}"
                        )

            srcs = result.get("sources", [])
            logger.info(f"Query completed with {len(srcs)} sources")
            return result
                    ...
                    out.append(
                        {
                            ...
                            if hasattr(n.node, "get_text")
                            else getattr(n.node, "text", ""),
                            ...
                            "chunk_id": getattr(n.node, "metadata", {}).get(
                                "chunk_id", ""
                            ),
                        }
                    )
                except Exception as e:
                    logger.debug("Skipping node due to formatting error: %s", e)
                    continue
            ...
            if use_reranking and self.reranker and out:
                try:
                    ...
                    reranked_results = self.reranker.rerank_documents(
                        query=query, documents=out, top_n=rerank_top_n
                    )
                    ...
                    for r in reranked_results:
                        doc = r.get("document")
                        if isinstance(doc, dict):
                            doc["relevance_score"] = r.get("relevance_score")
                            reranked_out.append(doc)

                    return reranked_out
                except Exception as e:
                    logger.warning(
                        f"Reranking failed, returning original results: {str(e)}"
                    )
