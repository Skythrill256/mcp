...
class HuggingFaceEmbeddingProvider(EmbeddingProvider):
    ...
    def __init__(self, config: AppSettings):
        ...
        if self.config.huggingface_api_key and not os.environ.get(
        ...
        try:
            logger.info(
                f"Loading Hugging Face model locally: {self.config.embedding_model}"
            )
            ...
            self.embed_model = SentenceTransformer(
                self.config.embedding_model, **model_kwargs
            )
            logger.info(f"Successfully loaded model: {self.config.embedding_model}")
        except Exception as e:
            logger.error(f"Failed to load Hugging Face model: {e}")
            raise EmbeddingError(
                f"Failed to load Hugging Face model '{self.config.embedding_model}': {e}"
            ) from e
    ...
    async def generate_embeddings(
        ...
        try:
            ...
            logger.debug(
                f"Texts to embed: {texts[:2]}..."
            )  # Log first 2 items for debugging

            embeddings = await asyncio.to_thread(self.embed_model.encode, texts)
            logger.info(
                f"Generated embeddings, count: {len(embeddings) if embeddings is not None else 0}"
            )
